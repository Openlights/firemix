...
class DlgSetupNetworking(QtWidgets.QDialog, Ui_DlgSetupNetworking):
    ...
    def __init__(self, parent=None):
        ...
        self.port_validator = QtWidgets.QIntValidator(1, 65535, self)
        ip_regex = QtCore.QRegExp(r"(?:(?:25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\.){3}(?:25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)")
        self.ip_validator = QtWidgets.QRegExpValidator(ip_regex, self)
    ...
    def validate_client_table_item(self, item):
        ...
        if valid == QtWidgets.QValidator.Invalid:
            item.setBackground(QtGui.QColor(255, 190, 190))
        elif valid == QtWidgets.QValidator.Intermediate:
            item.setBackground(QtGui.QColor(255, 255, 190))
        else:
            item.setBackground(QtGui.QColor(255, 255, 255))

        return (valid == QtWidgets.QValidator.Acceptable)
    ...
    def populate_clients_table(self):
        ...
        for i, client in enumerate(clients):
            item_ip = QTableWidgetItem(client["ip"])
            item_port = QTableWidgetItem(str(client["port"]))
        ...
        self.tbl_clients.horizontalHeader().setResizeMode(0, QtWidgets.QHeaderView.Stretch)

    def add_client_row(self):
        row = self.tbl_clients.rowCount()
        self.tbl_clients.insertRow(row)
        self.tbl_clients.setItem(row, 0, QTableWidgetItem(""))
        self.tbl_clients.setItem(row, 1, QTableWidgetItem("3020"))
        self.tbl_clients.setCellWidget(row, 2, QtWidgets.QCheckBox())
