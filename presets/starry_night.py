...
class StarryNight(RawPreset):
    """Starry night, based on Fireflies"""

    _fading_up = []
    _fading_down = []

    # Configurable parameters
    _up_target = (0.64, 1.0, 1.0)  # HSV
    _down_target = (0.60, 0.0, 0.0)  # HSV
    ...
    def _get_next_color(self, address, color, dt, down=False):
        ...
        delta = tuple(map(lambda x: x * progress, self._total_delta))
        if down:
            nc = tuple(map(lambda x, y: x - y, color, delta))
        else:
            nc = tuple(map(lambda x, y: x + y, color, delta))
        ...
        if (nc[0] < 0) or (nc[1] < 0) or (nc[2] < 0) or (nc[0] > 1) or (nc[1] > 1) or (nc[2] > 1):
            print "WARN", nc
            print "color", color
            print "total_delta", self._total_delta
            print "progress", progress
            print "delta", delta
